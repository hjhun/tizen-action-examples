...
- Base UI decisions primarily on the matching current Android Samsung stock app and recorded One UI adaptation evidence: Samsung Internet for Browser, Samsung Gallery for PhotoGallery, and relevant Samsung app/system components for generic A2UI surfaces. Preserve the Samsung mental model while adapting to Tizen input and viewport; do not invent a separate visual brand or label arbitrary gradients, floating docks, glass cards, generic dashboards, or invented navigation as One UI.
