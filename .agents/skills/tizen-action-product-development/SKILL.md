...
- Select the matching Android Samsung application wherever one exists (Samsung Internet for Browser, Samsung Gallery for PhotoGallery, Samsung Music for Music, and Samsung Video for Video), and record the exact app/screen/source/version/date and directly verified behavior.
...
- For a source-approved 1920×1080 translation example, inspect `Music/refs/music-design.html` read-only. Extract only its reusable translation traits (centered uniform reference canvas, TV-distance hierarchy/spacing, compact contextual chrome, content-first list/grid/detail composition, persistent task context when justified, two-cue focus, deterministic screen/input transitions). Never copy Music branding, rose/font/media tokens, playback/library controls, domain data, gradients, or exact geometry.
...
- Existing `surfaceUpdate` Template / `dataModelUpdate` Document output is a named legacy v0.8 compatibility profile. Preserve it for current producers, but do not call it v0.9.1. Canonical output must declare and validate an official Google A2UI version, lifecycle, catalog, semantic components, data bindings, and actions. Both paths must derive from the same current generated Entity snapshot and rendered state. For canonical work, record the upstream URL/revision/date/status and implement a separate negotiated version/catalog adapter: at `a2ui-project/a2ui` revision `ec97cb0d7499932e67003ffe5b709a3db7e7033a` (2026-08-07; inspected 2026-08-09), v0.9.1 is Current Production, v1.0 is Candidate, and v0.9.1 uses `createSurface`, `updateComponents`, `updateDataModel`, `deleteSurface`, plus client `action` messages.
- Treat catalog semantics and data as untrusted; the renderer owns native components, styling, layout bounds, accessibility, focus, and input dispatch. A2UI logical surfaces do not declare OS pixel formats, transparent windows, compositing, or input pass-through.
- `DisplayPresentation` is the repository's Google A2UI-compatible reference renderer. It preserves the validated A2UI semantic contract and maps supported components to reusable Samsung One UI-adapted NUI components/tokens, state, focus, and input—not flatten every payload into an arbitrary title/body card, invent a repository dialect, or allow payload-defined styling to bypass the renderer profile.
...
### Transparent overlay hosting

Separate (a) a 32-bit ARGB8888 buffer, (b) a native window with compositor-supported per-pixel alpha, and (c) an A2UI logical surface. Document evidence for each independently. API10 exposes `NUIApplication(string, WindowMode)`, `WindowMode.Transparent`, `Window.SetTransparency(bool)` (since Tizen 5), `Window.SetOpaqueState(bool)`, `Color.Transparent`, `Window.SetInputRegion(...)`, and `Window.SetAcceptFocus(...)`; these APIs do not by themselves prove target compositing or click-through.

Transparent overlay mode must be behind a runtime/build-target capability decision and have an opaque full-window fallback that preserves all product operations. On the installed Common Emulator, capture native screenshots with a known underlying app and verify fully transparent, semitransparent, and opaque regions; then exercise D-pad/key focus acquisition/trapping/restoration, pointer/touch inside and outside declared input regions, Back/dismiss, pause/resume, and fallback. Never promote host compile, ARGB8888 allocation, `SetTransparency(true)`, or a transparent View/root color to overlay-support evidence.
