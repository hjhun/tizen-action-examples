...
tizen build-cs -C "${CONFIGURATION:-Release}" -- "${BROWSER_DIR}/src/Browser.App"
BROWSER_STAGE="$(mktemp -d /tmp/browser-stage.XXXXXX)"
BROWSER_PACKAGE_OUTPUT="$(mktemp -d /tmp/browser-package.XXXXXX)"
trap 'rm -rf "$BROWSER_STAGE" "$BROWSER_PACKAGE_OUTPUT"' EXIT
BROWSER_OUTPUT="${BROWSER_DIR}/src/Browser.App/bin/${CONFIGURATION:-Release}/net8.0"
python3 - "$BROWSER_OUTPUT" "$BROWSER_STAGE" "$BROWSER_DIR" <<'PY'
...
shutil.copy2(app / 'src/Browser.App/tizen-manifest.xml', stage / 'tizen-manifest.xml')
PY
# Omitting -s intentionally selects Tizen Studio's emulator-only signer.
tizen package -t tpk -e "$BROWSER_STAGE" -o "$BROWSER_PACKAGE_OUTPUT" -- "$BROWSER_STAGE"
python3 - "$BROWSER_PACKAGE_OUTPUT" "$BROWSER_DIR" <<'PY'
from pathlib import Path
import shutil
import sys
import os
...
target = Path(os.environ.get('BROWSER_PACKAGE_OUTPUT', '/tmp/browser-packages/org.tizen.browser-0.1.0-api14.tpk'))
