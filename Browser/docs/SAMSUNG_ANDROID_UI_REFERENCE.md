...
| SAM-BROWSER-002 | [Use the navigation bar in Samsung Browser](https://www.samsung.com/us/support/answer/ANS10012955/) | Galaxy S26의 page/address/toolbar | 상단 address/search와 Reload, 하단 Back, Forward, Home, Browsing assist, Bookmarks, Tabs, Menu 계열을 직접 확인 | 페이지가 앱 version/One UI version을 명시하지 않음 |
| SAM-BROWSER-003 | [Open or close tabs in Samsung Browser](https://www.samsung.com/us/support/answer/ANS10012961/) | Tabs list/select/new/close/menu, close-all dialog | preview+title+URL card, selected outline, per-tab X, New tab, list/grid/stack, More options, `Cancel`/red `Close`를 직접 확인 | 개별 tab close의 확인 dialog 여부는 직접 확인되지 않음; close-all만 확인됨 |
...
|---|---|---|---|
| Page + navigation toolbar | 웹 콘텐츠가 대부분을 차지하고 address/Reload와 하단 Back/Forward/Home/Tabs/Menu 계열의 역할이 분리됨 | real `WebView`를 가장 큰 content region으로 두고 상단 address+Reload와 하단 Back/Forward/Tabs dock으로 분리 | 제어 의미와 역할 분리는 직접 검증; TV dock 크기·항목 축소는 adaptation |
| Address/search | 하나의 주소 영역으로 URL 탐색/검색하고 Reload가 인접함 | compact 상단 address/search, 초기 focus, Enter submit; URL이 아니면 privacy-safe search URL | 통합 주소/검색은 직접 검증; 검색 endpoint/geometry는 adaptation |
...
| Tabs list | 별도 Tabs screen, preview/title/URL, selected outline, X close, New tab | remote screenshot 대신 local privacy-safe preview block을 사용하고 제한 폭의 vertical cards로 TV-distance title/URL hierarchy를 강화 | 화면 hierarchy/selected/close는 직접 검증; local preview와 card geometry는 privacy/TV adaptation |
| Close all dialog | dimmed background, rounded modal, Cancel/red Close의 분할 action | 개별 tab close도 오작동 비용 때문에 동일한 modal family 사용; initial focus Cancel, Back cancel | dialog family는 직접 검증; 개별 tab confirm은 Tizen remote safety adaptation |
