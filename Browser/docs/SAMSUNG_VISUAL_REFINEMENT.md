...
- generated `HasPrivilegeLocal` RPC blocker와 canonical A2UI transport blocker, transport를 끊는 offline gate는 재수정·재실험하지 않고 차단 상태를 유지한다.
...
This completes only the Browser visual-fidelity module. Aurum returned an empty accessibility tree, so semantic accessibility remains unverified. Typed Browser/View RPC and dependent ViewAnnotation/legacy Display round trips remain blocked by the generated `HasPrivilegeLocal` runtime ABI mismatch. Canonical A2UI target rendering remains blocked by the two-string Presentation transport, and the transport-breaking offline experiment was not repeated.
