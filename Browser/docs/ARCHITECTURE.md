...
- **UseCases**: bounded URL/search normalization, immutable navigation phase, tab selection, lifecycle restoration, active-request cancellation, stale-result suppression, current agent-state/View registries, and typed result mapping. It accepts `CancellationToken` and has no Tizen dependency. Public URL projection removes user info, query, and fragment. Home/loading/error/Tabs/modal/paused state atomically suppresses the prior page Entity instead of publishing stale metadata.
...
- **ActionProvider**: generated full-category binding and thin validation/conversion/status adapter. Inputs are bounded before use. `GetBrowserByIds` preserves request order and duplicates and explicitly returns unresolved IDs. `GetCurrent` and `ToPresentation` read one atomic visible-state snapshot; a valid but non-current Entity is rejected. `Go` queues the validated URL against the current visible selected tab's stable ID, then rechecks that selection on the UI thread so an external Entity ID cannot detach current-state publication from the tab workspace.
- **ViewActionProvider**: generated full `Tizen.Action.View` binding over a lock-protected immutable measured-snapshot registry. Only the current visible normal-mode WebView is published. Each View uses a stable per-surface ID, actual positive finite NUI bounds, actual focused View identity, `Annotation.EntityType = Tizen.Entity.Browser`, and the generated Browser DTO `ToJson()` in `Annotation.EntityInfo`. `View_ToPresentation` resolves the live registry entry and rejects caller-forged annotation JSON.
- **A2UI producer and adapter**: [`A2UI_CONTRACT.md`](A2UI_CONTRACT.md) records the official v0.9.1 version/catalog/lifecycle and the separate legacy v0.8 DisplayPresentation adapter. Both derive from one bounded visible snapshot. The current two-string Tizen Presentation ABI cannot transport the canonical ordered stream, so the generated Actions return only the explicitly named legacy pair until a separately negotiated transport exists.
...
| View target | Annotated/focused views have parseable generated `EntityInfo`, finite bounds, FindById parity, and valid separate legacy compatibility Template/Document JSON; missing-ID is typed failure. |
| A2UI host/target | Official v0.9.1 messages validate against the recorded envelope/catalog schemas; legacy output passes the current DisplayPresentation parser. Canonical target render remains blocked until a negotiated Presentation transport exists. |
...
[`STAGE3_VALIDATION.md`](STAGE3_VALIDATION.md) is the authoritative package/Common Emulator ledger. The Browser-only product and UI path is installed and operational. The generator currently emits a call to `StubBase.HasPrivilegeLocal`, which is absent from the installed Public Tizen 10.1 RPCPort runtime. Earlier bindings used a post-generation compatibility experiment; current project policy prohibits reapplying it. A fresh unmodified binding on this runtime is a framework generator/runtime blocker, as recorded in [`RPCPORT_TIDLC_COMPATIBILITY.md`](RPCPORT_TIDLC_COMPATIBILITY.md).

In that historical experiment, all five Browser Actions and all four View Actions were exercised through target `action-tool`: current page retrieval, navigation, ordered resolution, presentation conversion, annotated/focused-view discovery, view lookup, and View-to-Presentation succeeded; the explicitly unsupported calendar handoff returned its typed `unavailable` result. The provider remained alive and no new Browser crash dump was observed. Canonical A2UI rendering remains independently blocked by the two-string Presentation transport.
