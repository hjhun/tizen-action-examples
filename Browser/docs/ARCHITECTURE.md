...
[`STAGE3_VALIDATION.md`](STAGE3_VALIDATION.md) is the authoritative package/Common Emulator ledger. The Browser-only product and UI path is installed and operational. The generator currently emits a call to `StubBase.HasPrivilegeLocal`, which is absent from the installed Public Tizen 10.1 RPCPort runtime. Until the planned `tidlc` fix lands, the generated C# UDS bindings use the documented fail-closed post-generation compatibility exception in [`RPCPORT_TIDLC_COMPATIBILITY.md`](RPCPORT_TIDLC_COMPATIBILITY.md).

With that exception, all five Browser Actions and all four View Actions were exercised through target `action-tool`: current page retrieval, navigation, ordered resolution, presentation conversion, annotated/focused-view discovery, view lookup, and View-to-Presentation succeeded; the explicitly unsupported calendar handoff returned its typed `unavailable` result. The provider remained alive and no new Browser crash dump was observed. Canonical A2UI rendering remains independently blocked by the two-string Presentation transport.
