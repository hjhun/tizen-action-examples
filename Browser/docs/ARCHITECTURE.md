...
- **Domain**: immutable page snapshots with stable, app-issued IDs; bounded normal-mode tab/history/bookmark collections; ordered duplicate-preserving ID resolution.
- **UseCases**: navigation request normalization, tab selection, history/bookmark queries, lifecycle restoration, stale-result suppression, and typed result mapping. It accepts `CancellationToken` and has no Tizen dependency.
- **Persistence**: one versioned, atomic JSON document for normal-mode session and bookmarks. Private-mode tabs, credentials, cookies, form contents, and raw page content are never persisted or exposed to Actions/annotations.
...
5. **History / bookmarks**: each has bounded searchable lists and a detail/context action. Removing a bookmark requires a focused confirmation overlay; Back cancels and restores the invoking focus.
6. **Calendar handoff**: `ToCalendar` is available only for a Browser Entity with enough page details to form a candidate. Unsupported extraction returns typed failure and a user-visible unavailable message; it does not invent event data.
...
| UI/Aurum | Home, load, error/retry, page, tab manager, history/bookmark, detail/confirmation, and focus states have native screenshot postconditions. Common Emulator evidence is reported separately from TV/product validation. |
