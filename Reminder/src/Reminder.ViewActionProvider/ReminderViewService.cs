...
public sealed class ReminderViewService : TizenActionView.ServiceBase
{
    ...
    public override TizenEntityStatus FindById(string id, out TizenEntityView v)
    {
        ...
        if (ReminderViewProviderState.TryFind(id, out v)) return Success();
        v = EmptyView();
    ...
    {
        views = ReminderViewProviderState.GetAnnotatedViews();
        return Success();
    }

    public override TizenEntityStatus GetFocusedView(out TizenEntityView v)
    {
        if (ReminderViewProviderState.TryGetFocused(out v)) return Success();
        v = EmptyView();
...
internal static class ReminderViewProviderState
{
