...
Collection/input limits are: 65,536 UTF-16 code units per JSON string, 512 semantic components, four text fields per visible page, 256 code units per displayed string, and one current rendering result. No presentation payload is persisted; process restart starts empty.
...
- `DisplayPresentation.ActionProvider` is a thin whole-category generated `Tizen.Action.Presentation` adapter. It maps generated Presentation DTOs to the use case and returns initialized typed Status values.
...
Catalog revision `4355566aa851470dc290c69672ba1f8bdd8470e7`, inspected 2026-09-06, establishes the following contracts:

- `Tv_Tizen.Action.Presentation_Show` is the sole entry in the `Tizen.Action.Presentation` `action.seq` section. Its input is `Tizen.Entity.Presentation`; output is `Tizen.Entity.Status`; the platform default provider is not reused by this app.
...
- The View category provides `FindById`, `GetAnnotatedViews`, `GetFocusedView`, and `ToPresentation`; the complete category is generated and checked against fresh actionc output. Generated files are never hand edited.

Each visible page uses View ID `display:<surface-id>:page:<zero-based-index>`; enabled renderer controls append `:control:<name>`. EntityType is `Tizen.Entity.Presentation` and EntityId is the accepted surface ID. EntityInfo contains only the current semantic page, excluding off-page values. View_ToPresentation rejects an old page ID or changed EntityInfo. The EntityInfo payload is the generated Presentation DTO `ToJson()` result, not a handwritten JSON projection. Bounds are only published after `CalculateScreenPositionSize()` gives finite positive values.
